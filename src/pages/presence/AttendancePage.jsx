...
// // 3. Remplacer la fonction fetchPresences par celle-ci
// const fetchPresences = useCallback(async (annee, mois) => {
//   if (presences.length === 0) setLoading(true);
//   else setRefreshing(true);

//   try {
//     // Étape 1 : récupérer les dates
//     const datesData = await presenceService.getDates(annee, mois);

//     // Construire la date du 20 du mois (format YYYY-MM-DD)
//     const expectedEndDateStr = `${annee}-${String(mois).padStart(2, '0')}-20`;

//     // Vérifier si la date du 20 est présente et hors_période = false
//     const hasEndDate = datesData.some(d => d.date === expectedEndDateStr && !d.hors_periode);

//     // Si elle manque et qu'on n'est pas déjà en train de régénérer
//     if (!hasEndDate && !isRegenerating.current) {
//       isRegenerating.current = true;
//       await presenceService.genererDates(annee, mois); // ← régénération
//       isRegenerating.current = false;
//       // Relancer le fetch après génération (sortie de la fonction, puis rappel)
//       return fetchPresences(annee, mois);
//     }

//     // Étape 2 : récupérer les présences avec les dates complètes
...
//     setPresences(presencesData.presences || []);
//     setPagination(presencesData.pagination || {
//       page: 1, page_size: rowsPerPage, total_employees: 0, total_pages: 0
//     });

//     // Reconstruire la map des événements
//     const newMap = {};
//     (presencesData.presences || []).forEach((p) => {
//       const d = formatDate(p.date);
//       if (!d || !p.evenement) return;
...
//     setRefreshing(false);
//   }
// }, [selectedSection, page, debouncedSearch, presences.length]); // dépendances
...
const AttendancePage = () => {
  ...
  const componentRef = useRef();
  const isFirstLoad = useRef(true); // ← pour distinguer premier chargement
  ...
  const fetchPresences = useCallback(async (annee, mois) => {
    ...
    try {
      ...
      const periodeDates = datesData ? datesData.filter(d => !d.hors_periode) : [];

      // Si le nombre de dates de la période est insuffisant → génération
      if (periodeDates.length < expectedDays) {
        console.log(`Génération des dates pour ${mois}/${annee} (attendues: ${expectedDays}, reçues: ${periodeDates.length})`);
  ...
  }, [selectedSection, page, debouncedSearch]); // ← plus de dépendance à presences
