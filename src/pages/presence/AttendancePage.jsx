...
// // import {
// //   Settings, Edit2, Plus, Save, X, Trash2, Search, ChevronDown, Clock
// // } from "lucide-react";
...
// //       if (searchRef.current && !searchRef.current.contains(e.target)) onFilter("");
// //     };
// //     document.addEventListener("mousedown", handleClickOutside);
// //     return () => document.removeEventListener("mousedown", handleClickOutside);
// //   }, [onFilter]);

// //   const handleChange = (e) => { setSearchQuery(e.target.value); onFilter(e.target.value); };
...
// //     <label className="text-sm font-bold text-gray-700 whitespace-nowrap">Section :</label>
// //     <div className="relative">
// //       <select
...
// //                   <div className="mt-1 font-semibold">{attendance?.section || employee?.section}</div>
// //                 </div>
// //               )}
// //             </div>
// //           </div>
...
// //                 {[
// //                   { label: "Brutes",     bg: "bg-gray-50",  color: "text-gray-600",  e: attendance.heure_brute_entree,     s: attendance.heure_brute_sortie },
// //                   { label: "Prévues",    bg: "bg-blue-50",  color: "text-blue-700",  e: attendance.heure_entree_prevue,    s: attendance.heure_sortie_prevue },
// //                   { label: "Rectifiées", bg: "bg-green-50", color: "text-green-700", e: attendance.heure_entree_rectifiee, s: attendance.heure_sortie_rectifiee },
// //                 ].map(({ label, bg, color, e, s }) => (
// //                   <div key={label} className={`text-center p-2 ${bg} rounded`}>
// //                     <div className={`font-bold ${color} mb-2`}>{label}</div>
// //                     <div className="text-xs text-gray-500">Entrée</div><div className="font-medium">{fmt(e)}</div>
// //                     <div className="text-xs text-gray-500 mt-2">Sortie</div><div className="font-medium">{fmt(s)}</div>
// //                   </div>
// //                 ))}
// //               </div>
// //             </div>
...
// //               >
// //                 {deleting
// //                   ? <><div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>Suppression...</>
// //                   : <><Trash2 className="w-4 h-4" />Supprimer</>}
// //               </button>
// //             )}
// //             <button onClick={onClose} disabled={loading || deleting} className="flex-1 px-4 py-3 border-2 border-gray-300 rounded hover:bg-gray-50">
// //               Annuler
// //             </button>
// //             <button
...
// //                 className="w-full px-3 py-2 border-2 border-gray-300 rounded focus:border-gray-800 focus:outline-none"
// //                 disabled={disabled}
// //                 placeholder={type === "text" ? "Ex: BRODERIE MAIN DEV" : undefined}
// //               />
...
// //             {[
// //               { label: "Sortie vendredi paiement *", key: "sortie_vendredi_paiement" },
// //               { label: "Sortie samedi paiement *",   key: "sortie_samedi_paiement" },
// //             ].map(({ label, key }) => (
...
// //                 <option key={t.code} value={t.code}>{t.libelle} ({t.code})</option>
// //               ))}
// //             </select>
// //           </div>
...
// //                 {[2,3,4,5,6,7].map((n) => <option key={n} value={String(n)}>CP{n} — {n} jours</option>)}
// //               </select>
...
// //                 Affiché : <strong>{autreTexte.trim() || "…"}</strong> ({autreTexte.length}/20)
// //               </p>
// //             </div>
// //           )}
...
// //               </button>
// //             )}
// //             <button onClick={onClose} disabled={saving || deleting} className="flex-1 px-4 py-2 border-2 border-gray-300 rounded hover:bg-gray-50">
// //               Annuler
// //             </button>
// //             <button
// //               onClick={handleSubmit}
...
// //   const handleMonthChange = (m) => { setSelectedMonth(m); setFilterChanged(m !== currentMonth || selectedYear !== currentYear); };
// //   const handleYearChange  = (y) => { setSelectedYear(y);  setFilterChanged(selectedMonth !== currentMonth || y !== currentYear); };
...
// //     try {
// //       const data = await presenceService.getSections();
...
// //     try {
// //       const data   = await presenceService.getEvenements(annee, mois);
// //       const newMap = {};
...
// //     try {
// //       setLoading(true);
// //       try { await presenceService.genererDates(annee, mois); } catch { /* silencieux */ }
...
// //     try {
// //       const data = await presenceService.getHorairesSection();
...
// //     fetchHoraires();
// //     fetchSections();
// //   }, [currentYear, currentMonth, forceRefresh, fetchPresences, fetchTypesEvenements, fetchHoraires, fetchSections]);
...
// //     const d = formatDate(dateStr);
// //     if (!d) return;
// //     const current = evenementsMap[`${employee.userid}-${d}`] || { type: "X", display: "", commentaire: "" };
...
// //     const hasE = !!(attendance.heure_brute_entree || attendance.heure_entree_comptabilisee);
// //     const hasS = !!(attendance.heure_brute_sortie || attendance.heure_sortie_comptabilisee);
...
// //   // ── Données employés avec filtre section ──
// //   const getEmployeeData = useCallback(() => {
// //     const employeesMap = {};

// //     presences.forEach((p) => {
// //       if (p.hors_periode) return;
// //       if (!p.present && !p.est_anomalie_corrigee && p.evenement === "X") return;
// //       // ── FILTRE SECTION ──
// //       if (selectedSection && p.section !== selectedSection) return;

// //       if (!employeesMap[p.userid]) {
// //         employeesMap[p.userid] = {
// //           userid:      p.userid,
...
// //       if (d) {
// //         employeesMap[p.userid].presences[d] = p;
// //         employeesMap[p.userid].hasPresence  = true;
// //       }
// //     });
...
// //     arr.sort((a, b) => a.badgenumber?.localeCompare(b.badgenumber, undefined, { numeric: true }) || 0);
// //     return arr;
// //   }, [presences, searchFilter, selectedSection]);
...
// //               <button
// //                 onClick={() => { setSelectedHoraire(null); setShowHoraireModal(true); }}
...
// //                     <button
// //                       onClick={() => { setSelectedHoraire(horaire); setShowHoraireModal(true); }}
...
// //                 </div>
// //                 {filterChanged && (
// //                   <button onClick={applyFilters} className="flex items-center gap-2 px-4 py-1 text-sm bg-akj text-white rounded mt-5">
// //                     Appliquer
// //                   </button>
// //                 )}
// //               </div>
...
// //                 {selectedSection || (employees.length > 0 ? employees[0].section : "TOUTES LES SECTIONS")}
// //               </span>
// //             </div>
...
// //             {/* Recherche */}
// //             <div className="h-10 flex items-center min-w-[200px]">
...
// //               onChange={(s) => { setSelectedSection(s); setSearchFilter(""); }}
// //             />

// //             {/* Compteur employés */}
// //             <div className="flex items-center px-3 py-1 bg-gray-100 rounded text-sm text-gray-600 border border-gray-300">
// //               <span className="font-semibold text-gray-800">{employees.length}</span>
// //               <span className="ml-1">employé{employees.length > 1 ? "s" : ""}</span>
// //             </div>
...
// //             <button
// //               onClick={() => navigate("/anomalies", { state: { month: currentMonth, year: currentYear, returnFromAnomalies: true } })}
...
// //                     <th key={weekNum} colSpan={weeks[weekNum].length} className="border border-gray-600 p-1 font-bold">
// //                       Semaine {weekNum}
// //                     </th>
// //                   ))}
// //                 </tr>
...
// //                         </div>
// //                         {/* Section visible uniquement en mode "toutes les sections" */}
// //                         {!selectedSection && (
// //                           <div className="text-xs text-gray-400 mt-0.5">{employee.section}</div>
// //                         )}
// //                       </td>
...
// //                         const estCorrectionAuto = !!(attendance?.est_correction_auto);
...
// //                               ></div>
// //                             )}
// //                             {/* Indicateur correction automatique */}
// //                             {estCorrectionAuto && (
// //                               <div
...
// //                                 title={modeHeures === "brutes" ? "Non modifiable" : "Modifier"}
// //                               >
// //                                 {heureEntreeAffichee
...
// //                                 title={eventData?.type ? `Événement: ${eventData.type}` : "Ajouter un événement"}
// //                               >
// //                                 {eventData?.display || ""}
// //                               </button>
...
// import {
//   Settings, Edit2, Plus, Save, X, Trash2, Search, ChevronDown, Clock, ChevronLeft, ChevronRight
// } from "lucide-react";
// import { useNavigate, useLocation } from "react-router-dom";
// import presenceService from "../../services/presenceService";
// import { useReactToPrint } from "react-to-print";
...
//   // Pagination backend : page UI commence à 0, backend à 1
...
//       }
//     }
//   }, [locationState, currentMonth, currentYear]);
...
//       const [datesData, presencesData] = await Promise.all([
//         presenceService.getDates(annee, mois),
//         presenceService.getPresencesMoisCalculee(
//           annee,
//           mois,
//           false,
//           selectedSection,
...
//       // Construire la map événements à partir des lignes visibles
//       const newMap = {};
...
//     } finally {
//       setLoading(false);
//       console.timeEnd("fetchPresences");
//     }
//   }, [selectedSection, page, debouncedSearch]);

//   // Chargement fixe une seule fois
//   useEffect(() => {
//     fetchTypesEvenements();
//     fetchHoraires();
//     fetchSections();
...
//   useEffect(() => {
//     fetchPresences(currentYear, currentMonth);
//   }, [currentYear, currentMonth, selectedSection, page, debouncedSearch, forceRefresh, fetchPresences]);
...
//   const datesValides = useMemo(() => dates.filter((d) => !d.hors_periode), [dates]);
...
//   // Les présences reçues sont déjà paginées côté backend
//   const employees = useMemo(() => {
//     console.time("buildEmployees");
//     const map = {};
...
//       .sort((a, b) => a.badgenumber?.localeCompare(b.badgenumber, undefined, { numeric: true }) || 0);

//     console.timeEnd("buildEmployees");
//     return arr;
//   }, [presences]);
...
//           <div className="flex flex-wrap gap-3 mt-4 border-t-2 border-gray-800 pt-4 print:hidden">
//             <div className="h-10 flex items-center min-w-[200px]">
//               <LocalEmployeeSearch value={searchFilter} onFilter={setSearchFilter} />
//             </div>
        ...
        <div className="p-6 space-y-4">
          ...
          {isCP && (
            <div className="bg-teal-50 border border-teal-200 rounded p-4">
              ...
              <p className="text-xs text-teal-600 mt-1">Affiché : <strong>{cpJours ? `CP${cpJours}` : "CP"}</strong></p>
            </div>
          )}
...
const AttendancePage = () => {
  ...
  const [periode,      setPeriode]      = useState(null);
  const [loading,      setLoading]      = useState(true);
  const [refreshing,   setRefreshing]   = useState(false); // ← spinner discret pour les rechargements
  const [pagination,   setPagination]   = useState({
    page: 1, page_size: 50, total_employees: 0, total_pages: 0, has_next: false, has_previous: false,
  });
  ...
  const fetchSections = useCallback(async () => {
    try { const d = await presenceService.getSections(); setSections(d.sections || []); }
    catch (err) { console.error("Erreur sections:", err); }
  }, []);

  const fetchTypesEvenements = useCallback(async () => {
    try { const d = await presenceService.getTypesEvenements(); setTypesEvenement(d.types_evenements || []); }
    catch (err) { console.error("Erreur types événements:", err); }
  }, []);

  const fetchHoraires = useCallback(async () => {
    try { const d = await presenceService.getHorairesSection(); setHoraires(d.horaires || []); }
    catch (err) { console.error("Erreur horaires:", err); }
  }, []);

  // ─── Fetch principal — SANS genererDates bloquant ─────────────────────────
  const fetchPresences = useCallback(async (annee, mois) => {
    // Premier chargement → spinner plein écran ; rechargements → spinner discret
    if (presences.length === 0) setLoading(true);
    else setRefreshing(true);

    try {
      // Charger dates + présences en parallèle (sans genererDates)
      const [datesData, presencesData] = await Promise.all([
        presenceService.getDates(annee, mois),
        presenceService.getPresencesMoisCalculee(
          annee, mois, false, selectedSection, page + 1, rowsPerPage, debouncedSearch
        ),
      ]);

      // Si aucune date → générer en arrière-plan (non bloquant) puis recharger
      if (!datesData || datesData.length === 0) {
        presenceService.genererDates(annee, mois)
          .then(() => setForceRefresh((p) => p + 1))
          .catch(console.error);
        setLoading(false);
        setRefreshing(false);
        return;
      ...
      setPagination(
        presencesData.pagination || {
          page: 1, page_size: rowsPerPage, total_employees: 0, total_pages: 0, has_next: false, has_previous: false,
        }
      );

      // Construire la map événements
      const newMap = {};
      (presencesData.presences || []).forEach((p) => {
        const d = formatDate(p.date);
  ...
  const handleEvenementClick = useCallback((employee, dateStr, attendance) => {
    ...
    const current = evenementsMap[`${employee.userid}-${d}`] || { type: "X", display: "", commentaire: "" };
    setSelectedEvenement({
      userid: employee.userid, badgenumber: employee.badgenumber,
      name: employee.name, date: d,
      type_evenement: current.type, commentaire: current.commentaire, attendance,
    });
    setShowEvenementModal(true);
  }, [evenementsMap]);
  ...
  const employees = useMemo(() => {
    ...
    presences.forEach((p) => {
      ...
      if (!map[p.userid]) {
        map[p.userid] = {
          userid: p.userid, badgenumber: p.badgenumber, name: p.name,
          section: p.section || "ADMINISTRATION", presences: {}, hasPresence: false,
        };
      }
      const d = formatDate(p.date);
      if (d) { map[p.userid].presences[d] = p; map[p.userid].hasPresence = true; }
    });
      ...
      <div ref={componentRef} className="print-container">
        ...
        <div className="bg-white border-2 border-gray-800 mb-4 p-6">
          ...
          <div className="flex flex-wrap gap-3 mt-4 border-t-2 border-gray-800 pt-4 print:hidden">
            ...
            <SectionSelector
              sections={sections}
              selectedSection={selectedSection}
              onChange={(s) => { setSelectedSection(s); setSearchFilter(""); setPage(0); }}
            />
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-end gap-4 mt-4 print:hidden">
              ...
              <span className="text-sm">Page {pagination.page || 1} / {totalPages}</span>
              <button
                onClick={() => setPage((p) => p + 1)}
                disabled={!pagination.has_next}
                className="flex items-center gap-1 px-3 py-1 border border-gray-300 rounded disabled:opacity-50"
        ...
        {employees.length === 0 && !loading && !refreshing && (
          <div className="bg-white border-2 border-gray-300 rounded p-12 text-center">
            ...
            {selectedSection && (
              <button
                onClick={() => { setSelectedSection(""); setPage(0); }}
                className="mt-4 px-4 py-2 bg-akj text-white rounded hover:bg-gray-700 text-sm"
              >
                Voir toutes les sections
              </button>
