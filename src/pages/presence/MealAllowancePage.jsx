...
const formatMoney = (value) =>
  new Intl.NumberFormat("fr-MG", { style: "currency", currency: "MGA" }).format(
    value,
  );
        ...
        <thead>
          <tr className="bg-gray-100">
            ...
            {semaineNums.map((s) => (
              <th
                key={s}
                className="border border-gray-600 py-3 px-1 text-center font-bold bg-gray-200 w-[70px]"
              >
                Semaine {s}
                <div className="text-xs font-normal text-gray-500">jours</div>
              </th>
            ))}
            <th className="border border-gray-600 py-3 px-1 text-center font-bold bg-gray-300 w-[60px]">
              Jours
            </th>
            <th className="border-2 border-gray-800 py-3 px-1 text-center font-bold bg-green-100 w-[110px]">
              Total (Ar)
            </th>
          </tr>
        </thead>
        <tbody>
          {employes.map((emp) => (
            ...
            >
              ...
              <td className="border border-gray-600 py-3 px-1 text-center font-bold bg-gray-50">
                {emp.total_jours}
              </td>
        ...
        <tfoot>
          <tr className="bg-gray-100 font-bold border-t-2 border-gray-800">
            ...
            {semaineNums.map((s) => (
              <td
                key={s}
                className="border border-gray-500 py-3 px-1 text-center"
              >
                {totaux[s]}
              </td>
            ))}
            <td className="border border-gray-600 py-3 px-1 text-center bg-gray-200">
              {totaux._total}
            </td>
      ...
      */}
      <table className="w-full border-collapse text-xs">
        <thead className="sticky top-0 z-20">
          ...
          <tr className="bg-gray-100">
            <th
              className="border-2 border-gray-800 p-2 text-left text-sm sticky left-0 bg-gray-100 z-30 w-[140px]"
              rowSpan={4}
            >
              Badge / Nom{!selectedSection && " / Section"}
            </th>
            {semaineNums.map((s) => {
              const count = datesList.filter(
                (d) => d.code_date?.charAt(0) === s,
              ).length;
              return (
                <th
                  key={s}
                  colSpan={count}
                  className="border border-gray-500 p-1 text-center font-bold bg-gray-200 text-sm"
                >
                  Semaine {s}
                </th>
              );
          ...
          <tr className="bg-gray-100">
            {datesList.map((d, idx) => (
              <th
                key={`code-${idx}`}
                className="border border-gray-600 p-1 text-center font-bold"
              >
                {d.code_date}
              </th>
            ))}
          ...
          <tr className="bg-gray-100">
            {datesList.map((d, idx) => (
              <th
                key={`mo-${idx}`}
                className="border border-gray-600 p-1 text-center text-xs text-gray-600 font-normal"
              >
                {new Date(d.date)
        ...
        <tbody>
          {employes.map((emp) => {
            const joursMap = new Map(
              emp.jours.map((j) => [j.date, j.indemnite]),
            );
            return (
              <tr
                key={emp.userid}
...
const MealAllowancePage = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const res = await presenceService.getIndemniteRepas(
        annee,
        mois,
        selectedSection,
        page,
      ...
      {data?.employes?.length === 0 && !loading && !refreshing && (
        <div className="bg-white border-2 border-gray-300 rounded p-12 text-center">
          ...
          <p className="text-gray-500 font-semibold">
            Aucune donnée d'indemnité pour cette période
          </p>
          {selectedSection && (
            <button
              onClick={() => {
